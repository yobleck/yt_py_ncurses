...
def user_subs():
    ...
    if(len(os.listdir(cwd + "json/yt_subs/")) == 0 or not os.path.isfile(cwd + "json/yt_subs/subs") or 
       num_subs > temp_num_subs or check_new_vids):
        ...
        if(check_new_vids):
            if(os.path.isfile(cwd + "json/yt_subs/subs_vid_count")):
                f = open(cwd + "json/yt_subs/subs_vid_count","r");
                temp_num_vids = json.load(f);#[int(i) for i in f.readlines()];
                f.close();
            else:
                temp_num_vids = {};
        ...
        while(True): #TODO: if len(["items"] < 50) then redo
            yt_sub_pages.append( yt_api.subscriptions().list(mine=True, part="snippet,contentDetails", order="alphabetical", 
                                                             maxResults=50, pageToken=temp_page_token).execute() ); #sub info more pages
        ...
        for i in yt_sub_pages:
            for j in i["items"]:
                ...
                subs_vid_count[j["snippet"]["title"]] = [j["contentDetails"]["totalItemCount"], False]; #number of vids and placeholder new vids
        f.close(); json.dump(subs_vid_count,f2); f2.close();
        
        
        #compare temp_num_vids to subs_vid_count and generate list has_new_vid
        if(check_new_vids):
            for key in subs_vid_count:
                if(key in temp_num_vids):
                    if(subs_vid_count[key][0] != temp_num_vids[key][0]):
                        subs_vid_count[key][1] = True;
        ...
        subs_vid_count = json.load(f2);#f2.readlines();
        f.close(); f2.close();
