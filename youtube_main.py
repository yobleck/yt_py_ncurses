...
#logging tool
#f = open("./log.txt","w"); f.write(str(test)); f.close();
...
def main(main_scr):
    ...
    try:
        ...
        main_scr.addstr(0,0,"Something went wrong. Attempting clean exit...");
        main_scr.refresh();
    ...
    yt_subs = [];
    if(len(os.listdir("./json/yt_subs/")) == 0):
        try:
            ...
            time.sleep(1); #TODO: change to larger value when done
            curses.endwin();
            return "could not get subscription information";
        
        f = open("./json/yt_subs/subs","w");
    ...
    sub_pad = curses.newpad(num_subs+1,term_w);
    chnl_pad = curses.newpad(num_vids,term_w);
    vid_scr = curses.newwin(term_h-1,term_w,1,0);
    ...
    sub_pad.addstr(0,0, "My YT Subscriptions", curses.A_ITALIC); #TODO: add notification of new videos on this page?
    ...
    while(loop):
        ...
        #if(usr_input == 118 and is_visible[1] and is_visible[3]): #v
            #is_visible[4] = True; in_focus = fake_panel[4];
        ...
        if(usr_input == 10):
            if(in_focus == fake_panel[2]): #selecting channel to display uploads
                ...
                fake_panel[3][0].addnstr(0,0, "Channel: " + yt_subs[in_focus[4]]["title"], term_w, curses.A_ITALIC);
                fake_panel[3][5] = [];
                ...
                fake_panel[4][0].addnstr(0,0,"Video: " + in_focus[5][in_focus[4]]["title"], term_w);
                fake_panel[4][0].addnstr(1,0,"Desc: " + in_focus[5][in_focus[4]]["description"].strip("\n"), term_w);
        ...
        for x in range(5):
            if(fake_panel[x] == in_focus):#if(is_visible[x]):
                ...
                try:
                    ...
                    fake_panel[x][0].refresh(fake_panel[x][1],fake_panel[x][2], 1,0, term_h-1,term_w); #pads
